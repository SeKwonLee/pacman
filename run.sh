...
do
	...
	sudo rm -rf /mnt/pmem0/*
        sudo numactl --membind=0 --cpunodebind=0 ./example/ycsb_flatstore 50331648 50331648 256 ${workload} ${nthreads} 4 &>> ../results/flatstore_scale_results.txt
        printf "\n" &>> ../results/flatstore_scale_results.txt
    done
done
...
do
    ...
    do
	sudo rm -rf /mnt/pmem0/*
        sudo numactl --membind=0 --cpunodebind=0 ./benchmarks/other/ycsb_viper 50331648 50331648 256 ${workload} ${nthreads} 4 &>> ../results/viper_scale_results.txt
        printf "\n" &>> ../results/viper_scale_results.txt
    done
done
...
do
    ...
    do
        sudo rm -rf /mnt/pmem0/*
        sudo numactl --membind=0 --cpunodebind=0 ./benchmarks/other/ycsb_chameleondb 50331648 50331648 256 ${workload} ${nthreads} 4 &>> ../results/chameleondb_scale_results.txt
        printf "\n" &>> ../results/chameleondb_scale_results.txt
    done
done
...
#    do
#        sudo rm -rf /mnt/pmem0/*
#        sudo numactl --membind=0 --cpunodebind=0 ./ycsb_chameleondb 50331648 50331648 1024 ${workload} ${nthreads} 4 &>> ../../../chameleondb_scale_1KB_results.txt
#        printf "\n" &>> ../../../chameleondb_scale_1KB_results.txt
#    done
#done

#kvs_name=$1
#for valuesize in 256 1024 4096
#do
#    for nthreads in 1 2 4 8 16 32 44
#    do
#        sudo rm -rf /mnt/pmem0/*
#        sudo numactl --membind=0 --cpunodebind=0 ./ycsb_${kvs_name} 50331648 50331648 ${valuesize} a ${nthreads} 4 &>> /home/cc/pacman/${kvs_name}_scale_${valuesize}_results.txt
